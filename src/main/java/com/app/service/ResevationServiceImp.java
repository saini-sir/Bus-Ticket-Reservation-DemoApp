...
import com.app.Utilities.PDFgenerator;
...
public class ResevationServiceImp implements ReservationService {
	...
	@Override
	public Reservations addReservation(ReservationDTO reservationDTO,String key ) throws ReservationException, LoginException, MessagingException {
		...
		if(!reservationDTO.getDestination().equalsIgnoreCase(opt.get().getRouteTo()))
			...
			int Uuid =random.nextInt(90000000)+10000000;
...
//			user.setRervation(userReservation);
		...
		pdf.PDFgenerator(filePath+reservation.getReservationId()+".pdf",user.getUserLoginId(), user.getEmail(),
				user.getContact(), upadateBus.getBusId(),reservationDTO.getJourneyDate(),reservationDTO.getSource(),opt.get().getRouteTo(),
				reservationDTO.getNoOfSeatsToBook(), reservation.getReservationStatus(),(opt.get().getFarePerSeat())*(reservationDTO.getNoOfSeatsToBook()),
				reservation.getReservationType());
		emailService.sendMailwithAttach(
				user.getEmail(),
				"Reservation Update",
				"Your reservation is successfully completed and your Reservation Id is: " + reservation.getReservationId() + ", " + reservation.getReservationType() + ", " + "Journey Date: " + reservation.getJourneyDate() + ", " + "Reservation Date: " + LocalDateTime.now(),
				filePath + reservation.getReservationId() + ".pdf"
		);
		return reservation;
    }
